...
export default async function handler(req, res) {
  ...
  // Only handle PUT for description update and other fields
  if (req.method === "PUT") {
    // Accept id from either URL (req.query.id), body.id, or body._id
    let id = req.query.id;
    if (!id && req.body) id = req.body._id || req.body.id;
    const payload = req.body;
    if (!id || typeof id !== "string" || id.trim() === "" || !payload || typeof payload !== 'object') {
      return safeJson(res, 400, { error: "Missing id or payload" });
    }
    ...
    const allowed = {};
    ['title', 'price', 'image', 'category'].forEach(k => {
      if (payload[k] !== undefined) allowed[k] = payload[k];
    });
    // Always update both description and desc for compatibility
    if (payload.description !== undefined) {
      allowed.description = payload.description;
      allowed.desc = payload.description;
    } else if (payload.desc !== undefined) {
      allowed.description = payload.desc;
      allowed.desc = payload.desc;
    }
    if (Object.keys(allowed).length === 0) {
      return safeJson(res, 400, { error: "No updatable fields in payload" });
    }
    ...
    await client.close();
    return safeJson(res, 200, updated);
  }
